...
public interface RecruiterApplicationApi {
    ...
    ResponseEntity<CandidateApplicationResponse> sendApplication(
    ...
    @ApiOperation(notes = "Get all applications sent to candidate", value = "Get all applications sent to candidate", response = CandidateApplicationResponse.class,
    ...
    ResponseEntity<List<CandidateApplicationResponse>> getCandidateApplications(
            @ApiParam(name = CANDIDATE_ID, value = CANDIDATE_ID_DESCRIPTION, required = true) @PathVariable(name = CANDIDATE_ID) String candidateId);

    @ApiOperation(notes = "Get all applications to some vacancy", value = "Get all applications to some vacancy", response = CandidateApplicationResponse.class,
            responseContainer = LIST)
    @ApiResponses({
            @ApiResponse(code = 200, message = "All applications"),
            @ApiResponse(code = 400, message = ConstantsUtils.HTTP_400_MESSAGE),
    ...
    ResponseEntity<List<VacancyApplicationResponse>> getVacancyApplications(
            @ApiParam(name = VACANCY_ID, value = VACANCY_ID_DESCRIPTION, required = true) @PathVariable(name = VACANCY_ID) String vacancyId);
}
